...
The core pipeline (preprocess, valence, dictionaries, aggregate) runs on
a standard R installation with no Java and no Python. Two parts are
optional and carry extra requirements:

- **Spell-checking in preprocessing.** Only relevant when
  `spellcheck = TRUE`. The default `"hunspell"` method needs Java and a
  WordNet dictionary; the `"gemini"` method needs a Gemini API key. Set
  `spellcheck = FALSE` to skip the step and require neither.
- **Embeddings and seed similarities (Stages 4 and 5).** Need Python
  through `reticulate` (SBERT uses the `sentence-transformers` library;
  Gemini embeddings use Google’s `genai` library plus an API key). These
  stages are optional, and the dictionary-based coding is complete
  without them.
...
Spell-checking is optional and is governed by two arguments:
`spellcheck` (whether to run it at all) and `spellcheck_method` (how to
run it, when it runs).

Set `spellcheck = FALSE` to skip spell-checking entirely. This requires
neither Java nor an API key, and is the simplest option if your
responses are already clean or you just want a first pass. When skipped,
`tv2` is copied directly from `tv`.

When `spellcheck = TRUE`, two methods are available via
`spellcheck_method`:
